...
static void errmsg();
static int  *force_solaris_bus_error = (int *)1;	/* force core dump because of a detected error */
		...
		** database.  Simply create a record and link it in */
		...
		if(cmpval < 0) {
			...
			** same for every record on it. */
			...
			size = ((sizeof(struct dbrec) + idb.hdrp->wordsz + 4) /4) *4;
...
static int newdbrec(int wordlen, struct idbifc *idbifc)
{
	...
	size = ((((sizeof(struct dbrec) + wordlen + 1)) + 3) /4) *4;	/* for solaris */ 
	...
	idbifc->bodyp = (struct dbrecbody *)((char *)hdr + (((sizeof(struct dbrechdr) + wordlen + 4) /4) *4));
...
struct idbifc *idbifc;
{
	...
	sizebody = (sizeof(struct dbrecbody) + sizeof(body->entry[0]) * (nentries - NUMDBRENTRIES));	/* for solaris */
...
*/
		...
		size += read(idbifc->fd, &idb.bodyp->nxtwrd, sizeof(struct dbrecbody));
...
static int putdbrec(struct idbifc *idbifc, int size)
{
	/*  Solaris requires that proper alignment boundaries are kept or bus errors happen.
	**  This hurts bodyp because the database aligns it adjacent to wordsz which is variable in size
	**  Elsewhere in this code the alignment is taken care of when data is read from the DB, now data is to be written
	**  We must mangle the data back down to it's more condensed form
	*/
	
	char	*dbrec_ptr;
	int	total_size, hdr_size, tmp; 
	int	*ptr_int = 1; /* force sparc/solaris bus error */
	total_size = sizeof(struct dbrechdr) + sizeof(struct dbrecbody) +  idbifc->hdrp->wordsz + 1;
	hdr_size   = sizeof(struct dbrechdr) + idbifc->hdrp->wordsz + 1;
	
	dbrec_ptr = mymalloc(total_size);
	memset(dbrec_ptr, 0, total_size);
	
	memcpy(dbrec_ptr, idbifc->hdrp, sizeof(struct dbrechdr) + idbifc->hdrp->wordsz + 1);
	memcpy(dbrec_ptr + sizeof(struct dbrechdr) + idbifc->hdrp->wordsz + 1, idbifc->bodyp, sizeof(struct dbrecbody));

	/* tmp = *ptr_int;		/* barf it to have a look */

	lseek(idbifc->fd, idbifc->pos, 0);

	/* play with sizes, the calling routines expect to be returned the amount of data written (in bytes) */

	if ( write(idbifc->fd, dbrec_ptr, total_size) == total_size)
	{
	    idbifc->size = size;
	    addmembuf(idbifc, idbifc->size);
	}
...
static int addmembuf(struct idbifc *idbifc,int  rcdsize)
{
	...
	**/
	...
	if (lastbp->length & 1)
	{
	    /* shouldn't have odd length, it forces the next buffer to be mis aligned */

	    *force_solaris_bus_error = matched;
...
static int getmembuf(struct idbifc *idbifc)
{
	...
	**/
	...
	if(lbp > lastbp || (!lbp) || lbp->pos > idbifc->pos)
		lbp = bufhead;
    ...
    **/
	...
	idbifc->bodyp = (struct dbrecbody *)((char *)hdr
					+ (((sizeof(struct dbrechdr) + hdr->wordsz + 4) /4)*4));
	idbifc->hdrp = hdr;
